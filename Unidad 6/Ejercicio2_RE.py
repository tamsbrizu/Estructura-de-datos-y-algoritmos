...
class digrafo_encadenado:
    ...
    def insertar (self, v1, v2):
        ...
        nuevoNodo = Nodo(v)
        nuevoNodo.set_sig(self.__tabla[u])
        self.__tabla[u] = nuevoNodo
    ...
    def grado_salida (self, v1):
        ...
        for j in range(self.__tamanio):
            if self.verificarAdyacente(u, j):
                cont += 1
        return cont
    ...
    def grado_entrada (self, v1):
        ...
        for i in range(self.__tamanio):
            if self.verificarAdyacente(i, u):
                cont += 1
        return cont
...
if __name__ == "__main__":
    ...
    # Creamos un grafo dirigido de 5 nodos
    g = digrafo_encadenado(5)

    # Insertamos aristas dirigidas
    g.insertar(1, 2)
    g.insertar(2, 3)
    g.insertar(3, 4)
    g.insertar(4, 5)
    g.insertar(5, 2)   # Esto genera un ciclo
    
    print("\n--- LISTA DE ADYACENCIA ---")
    for i in range(1, 6):
        print(f"{i} -> {g.obtenerAdyacentes(i)}")

    print("\n--- CONECTIVIDAD ---")
    print("Fuertemente conexo?:", g.fuertemente_conexo())
    print("Simplemente conexo?:", g.simple_conexo())

    print("\n--- CICLOS ---")
    print("Es acíclico?:", g.es_aciclico())

    print("\n--- CAMINOS (BFS) ---")
    g.camino(1, 5)
    print()
    g.camino(5, 1)  # No existe camino
    ...
    print("\n--- GRADOS ---")
    for i in range(1, 6):
        print(f"Nodo {i}: Entrada={g.grado_entrada(i)}, Salida={g.grado_salida(i)}")

    print("\n--- FUENTE / SUMIDERO ---")
    for i in range(1, 6):
        if g.nodo_fuente(i):
            print(f"{i} es nodo fuente")
        if g.nodo_sumidero(i):
            print(f"{i} es nodo sumidero")
